// PluginsInfo.jsx / StudioSaleSection.jsx
import React from "react";

import serumimg from "../img/serum2-hero-min.png";
import nexus5 from "../img/nexus_5.png";
import wavesImg from "../img/waves_bundle.jpg";
import fabfilterImg from "../img/fabfilter_pro_bundle.jpg";
import vsthero from "../img/vsthero.png";

export default function StudioSaleSection() {
  return (
    <section className="w-full bg-gradient-to-br from-black via-black to-orange-800/30 text-white py-24">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        {/* Верхний блок */}
        <div className="grid gap-10 md:grid-cols-2 items-center mb-20">
          <div className="space-y-6">
            <p className="text-xs uppercase tracking-[0.25em] text-orange-400">
              phase plugins
            </p>
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold leading-tight tracking-tight uppercase">
              МЫ ТЕПЕРЬ РАЗРАБАТЫВАЕМ
              <br />
              <span className="text-orange-500">ПЛАГИНЫ</span>
            </h1>

            <p className="text-lg md:text-xl max-w-xl text-gray-200">
              Создаём плагины для сведения, синтезаторы и эффекты, которые сами
              используем в работе на студии. Ранняя стоимость ниже релизной.
            </p>

            <p className="text-sm md:text-base font-semibold tracking-wide uppercase text-orange-400">
              Акция на пресейл действует до 4 декабря.
            </p>
          </div>

          {/* Картинка справа */}
          <div className="relative flex justify-center md:justify-end">
            <div className="relative w-full max-w-md">
              <div className="absolute -inset-6 rounded-3xl bg-orange-500/15 blur-3xl opacity-80" />
              <div className="relative rounded-3xl overflow-hidden ">
                <img
                  src={vsthero}
                  alt="Инструменты и плагины"
                  className="w-full h-full object-contain"
                />
              </div>
            </div>
          </div>
        </div>

        {/* Карточки продуктов */}
        <div className="grid gap-6 md:grid-cols-4">
          {/* SERUM */}
          <div className="flex flex-col bg-black/85 border border-orange-500/30 rounded-2xl overflow-hidden">
            <div className="px-5 pt-5 pb-4 bg-gradient-to-b from-orange-900/50 to-black">
              <div
                className="h-40 rounded-xl mb-4"
                style={{
                  backgroundImage: `url(${serumimg})`,
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                }}
              />
              <p className="text-xs font-bold tracking-wide text-orange-400 uppercase">
                скидка 60%
              </p>
              <h3 className="mt-1 text-sm font-extrabold tracking-wide uppercase">
                SERUM 2
              </h3>
              <p className="mt-2 text-xs text-gray-300">
                Таблично-волновой синтезатор с новыми режимами грануляции и
                улучшенным движком.
              </p>
            </div>
            <div className="mt-auto px-5 pb-5 pt-3">
              <button className="w-full py-3 bg-orange-500 hover:bg-orange-600 transition text-sm font-bold uppercase rounded-xl">
                КУПИТЬ
              </button>
            </div>
          </div>

          {/* FABFILTER */}
          <div className="flex flex-col bg-black/85 border border-orange-500/30 rounded-2xl overflow-hidden">
            <div className="px-5 pt-5 pb-4 bg-gradient-to-b from-orange-700/60 to-black">
              <div
                className="h-40 rounded-xl mb-4"
                style={{
                  backgroundImage: `url(${fabfilterImg})`,
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                }}
              />
              <p className="text-xs font-bold tracking-wide text-orange-300 uppercase">
                скидка 60%
              </p>
              <h3 className="mt-1 text-sm font-extrabold tracking-wide uppercase">
                FABFILTER BUNDLE
              </h3>
              <p className="mt-2 text-xs text-gray-300">
                Полный набор эквалайзеров, компрессоров и лимитеров для
                профессионального микса.
              </p>
            </div>
            <div className="mt-auto px-5 pb-5 pt-3">
              <button className="w-full py-3 bg-orange-500 hover:bg-orange-600 transition text-sm font-bold uppercase rounded-xl">
                КУПИТЬ
              </button>
            </div>
          </div>

          {/* WAVES */}
          <div className="flex flex-col bg-black/85 border border-orange-500/30 rounded-2xl overflow-hidden">
            <div className="px-5 pt-5 pb-4 bg-gradient-to-b from-orange-600/60 to-black">
              <div
                className="h-40 rounded-xl mb-4"
                style={{
                  backgroundImage: `url(${wavesImg})`,
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                }}
              />
              <p className="text-xs font-bold tracking-wide text-orange-300 uppercase">
                скидка 60%
              </p>
              <h3 className="mt-1 text-sm font-extrabold tracking-wide uppercase">
                WAVES BUNDLE
              </h3>
              <p className="mt-2 text-xs text-gray-300">
                Классические плагины и эмуляции легендарных железных
                компрессоров и эквалайзеров.
              </p>
            </div>
            <div className="mt-auto px-5 pb-5 pt-3">
              <button className="w-full py-3 bg-orange-500 hover:bg-orange-600 transition text-sm font-bold uppercase rounded-xl">
                КУПИТЬ
              </button>
            </div>
          </div>

          {/* NEXUS */}
          <div className="flex flex-col bg-black/85 border border-orange-500/30 rounded-2xl overflow-hidden">
            <div className="px-5 pt-5 pb-4 bg-gradient-to-b from-zinc-900 to-black">
              <div
                className="h-40 rounded-xl mb-4"
                style={{
                  backgroundImage: `url(${nexus5})`,
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                }}
              />
              <p className="text-xs font-bold tracking-wide text-orange-300 uppercase">
                скидка 60%
              </p>
              <h3 className="mt-1 text-sm font-extrabold tracking-wide uppercase">
                NEXUS 5
              </h3>
              <p className="mt-2 text-xs text-gray-300">
                ROMpler и синтезатор с большим количеством готовых пресетов
                под любой жанр.
              </p>
            </div>
            <div className="mt-auto px-5 pb-5 pt-3">
              <button className="w-full py-3 bg-orange-500 hover:bg-orange-600 transition text-sm font-bold uppercase rounded-xl">
                КУПИТЬ
              </button>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
